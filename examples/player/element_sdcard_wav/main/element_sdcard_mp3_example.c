...
void app_main(void)
{
    ...
    QueueHandle_t i2s_queue, wav_queue;
    ...
    ESP_LOGI(TAG, "[3.1] Create wav decoder to decode wav data");
    wav_decoder_cfg_t wav_cfg = DEFAULT_WAV_DECODER_CONFIG();
    wav_decoder = wav_decoder_init(&wav_cfg);

    ESP_LOGI(TAG, "[3.2] Associate custom read callback to wav decoder");
    audio_element_set_read_cb(wav_decoder, my_read_cb, NULL);

    ESP_LOGI(TAG, "[3.3] Create a ringbuffer and insert it between wav decoder and i2s writer");
    ...
    if (xQueueRemoveFromSet(wav_queue, queue_set) != pdPASS) {
        ESP_LOGE(TAG, "Error remove wav_queue from queue_set");
    }
