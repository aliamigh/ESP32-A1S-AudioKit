...
#define WELLCOME_FILE     "/sdcard/WELLCOME.wav"
...
#define CODEC_VOLOM 50

#define VAD_SAMPLE_RATE_HZ 16000
#define FINALE_FILE_SAVE_HZ VAD_SAMPLE_RATE_HZ
#define VAD_FRAME_LENGTH_MS 30
#define VAD_BUFFER_LENGTH (VAD_FRAME_LENGTH_MS * VAD_SAMPLE_RATE_HZ / 1000)
#define MAX_SEQ   50
#define MIDLE_LEN   MAX_SEQ*VAD_BUFFER_LENGTH
#define SEQ_CHACH_BUF 10
#define VAD_COUNTER_SENSITIVTY 2
#define VED_CHECKING_COUNT 10
#define VAD_MODE VAD_MODE_1
...
audio_element_info_t info_down_sampler = AUDIO_ELEMENT_INFO_DEFAULT();
...
void play_wav(char * file_name_wav)
{
    audio_pipeline_handle_t pipeline;
    ...
    audio_element_set_uri(fatfs_stream_reader,file_name_wav);
...
static bool sendBody(esp_http_client_handle_t client, const char *fileName, int *ahang1Salam2No3, int *ahangNumber)
{
    ...
    *ahang1Salam2No3 = 3;
    ...
    if (status==200){
        if(responseLength<=2){
            *ahang1Salam2No3 = 1;
            ESP_LOGW(TAG_HTTP, "**************Ahang***************************");
        }else{
            *ahang1Salam2No3 = 2;
            ESP_LOGW(TAG_HTTP, "**************Salam****************************");
        }
    }else if(status==400 && responseLength==43){
        *ahang1Salam2No3 = 3;
        ESP_LOGW(TAG_HTTP, "***************No ahang No salam****************");
    }else{
        *ahang1Salam2No3 = 4;
...
void reocrd_vad_wav_file(char * buffer)
{
  
    audio_pipeline_handle_t pipeline1;
    audio_element_handle_t  filter, raw_read;
    audio_element_handle_t equalizer,i2s_stream_writer;
    ...
    // i2s_cfg1.i2s_config.channel_format = I2S_CHANNEL_FMT_ALL_LEFT;
 
    i2s_stream_reader = i2s_stream_init(&i2s_cfg1);

    // ESP_LOGI(TAG_REC, "[2.2] Create filter to resample audio data");
    // rsp_filter_cfg_t rsp_cfg = DEFAULT_RESAMPLE_FILTER_CONFIG();
    // rsp_cfg.src_rate = VAD_SAMPLE_RATE_HZ;
    // rsp_cfg.src_ch = 1;
    // rsp_cfg.dest_rate = FINALE_FILE_SAVE_HZ;
    // rsp_cfg.dest_ch = 1;
    ...
    // audio_element_getinfo(i2s_stream_reader, &info);
    audio_element_setinfo(fatfs_stream_writer, &info_down_sampler);
 

    ESP_LOGI(TAG_REC, "[2.3] Create raw to receive data");
    raw_stream_cfg_t raw_cfg1 = {
        // .out_rb_size = MAX_SEQ * VAD_BUFFER_LENGTH * sizeof(short),
        .out_rb_size = 30*1024,
    ...
    ESP_LOGI(TAG_REC, "[ 3 ] Register all elements to audio pipeline1");
    ESP_LOGI(TAG_REC, "[ 4 ] Link elements together >raw-->filter-->wav--->fatfs");

    ESP_LOGI(TAG_REC, "[ 3 ] Register all elements to audio pipeline2");
    audio_pipeline_register(pipeline2, raw_write, "raw");
    // audio_pipeline_register(pipeline2, filter, "filter");
    ...
    // const char *link_tag2[4] = {"raw","filter","wav", "fatfs"};
    // audio_pipeline_link(pipeline2, &link_tag2[0], 4);
    ESP_LOGI(TAG_REC, "[ 4 ] Link elements together raw-->wav-->fatfs");
    ...
    vad_handle_t vad_inst = vad_create(VAD_MODE, VAD_SAMPLE_RATE_HZ, VAD_FRAME_LENGTH_MS);
    ...
    periph_led_blink(led_handle, get_green_led_gpio(), 200, 200, true, -1,PERIPH_LED_IDLE_LEVEL_HIGH);

    int rec_seq = 0;
    while (1) { 
        raw_stream_read(raw_read,buffer,SEQ_CHACH_BUF* VAD_BUFFER_LENGTH * sizeof(short));
        raw_stream_read(raw_read,buffer+SEQ_CHACH_BUF* VAD_BUFFER_LENGTH * sizeof(short),VAD_BUFFER_LENGTH * sizeof(short));

        vad_state = vad_process(vad_inst,buffer+SEQ_CHACH_BUF* VAD_BUFFER_LENGTH * sizeof(short));
        if (vad_state == VAD_SPEECH) {
            ESP_LOGI(TAG_REC, "start");
            vad_num_speech = 0;
            int rec_seq = SEQ_CHACH_BUF+1;
            for(;rec_seq<SEQ_CHACH_BUF+1+VED_CHECKING_COUNT;rec_seq++){
                raw_stream_read(raw_read,buffer+rec_seq* VAD_BUFFER_LENGTH * sizeof(short),VAD_BUFFER_LENGTH * sizeof(short));
                vad_state = vad_process(vad_inst,buffer+rec_seq* VAD_BUFFER_LENGTH * sizeof(short));
                if(vad_state==VAD_SPEECH){
            #ifdef DEBUG
                    ESP_LOGI(TAG_REC, "+");
            #endif
                ...
                if(vad_num_speech<0){
                ESP_LOGE(TAG_REC, "failed");
                    break;
                }
            }
            if(vad_num_speech>VAD_COUNTER_SENSITIVTY){
                raw_stream_write(raw_write, (char *)buffer,rec_seq* VAD_BUFFER_LENGTH * sizeof(short));
    ...
    audio_pipeline_unregister(pipeline2, raw_write);
    // audio_pipeline_unregister(pipeline2, filter);
    audio_pipeline_unregister(pipeline2, wav_encoder);
...
void app_main(void){
    ...
    periph_led_blink(led_handle, get_blue_led_gpio(), 10, 10, false, -1, 1);
    ...
    audio_hal_ctrl_codec(board_handle->audio_hal, AUDIO_HAL_CODEC_MODE_BOTH, AUDIO_HAL_CTRL_START);
    audio_hal_set_volume(board_handle->audio_hal,CODEC_VOLOM);


    ESP_LOGI(TAG_REC, "[2.1] Create i2s stream to read audio data from codec chip");
    i2s_stream_cfg_t i2s_cfg1 = I2S_STREAM_CFG_DEFAULT();
    i2s_cfg1.i2s_config.sample_rate = FINALE_FILE_SAVE_HZ;
    i2s_cfg1.type = AUDIO_STREAM_READER;
    i2s_cfg1.i2s_config.channel_format = I2S_CHANNEL_FMT_ONLY_LEFT;
    audio_element_handle_t i2s_stream_reader2 = i2s_stream_init(&i2s_cfg1);

    audio_element_getinfo(i2s_stream_reader2, &info_down_sampler);
    audio_element_deinit(i2s_stream_reader2);
    ...
    while(1){
        ...
        recording_process();
         ...
         periph_led_blink(led_handle, get_green_led_gpio(), 10, 10, false, -1, 0);
